...
tags:
  - "clippings"
---
A trigger is what wakes an automation up. Until something triggers it, an automation just sits there quietly, waiting. The moment a trigger fires, Home Assistant checks any [conditions](https://www.home-assistant.io/docs/automation/condition/) you set, and if they pass, it runs the [actions](https://www.home-assistant.io/docs/automation/action/).
...
![Play: How to use Trigger IDs in Home Assistant - Tutorial](https://i.ytimg.com/vi_webp/fE_MYcXYwMI/maxresdefault.webp)
...
The second variant is setting variables that are available when attaching a trigger when the trigger can contain templated values. These are defined using the `trigger_variables` key at an automation level. These variables can only contain [limited templates](https://www.home-assistant.io/docs/templating/where-to-use/#limited-templates). The triggers will not re-apply if the value of the template changes. Trigger variables are a feature meant to support using blueprint inputs in triggers.
...
For setup steps, YAML options, and examples for the event trigger, see [Event trigger](https://www.home-assistant.io/triggers/event/).

## Home Assistant trigger

For setup steps, YAML options, and examples for the Home Assistant trigger, see [Home Assistant trigger](https://www.home-assistant.io/triggers/homeassistant/).
...
It’s also possible to use [limited templates](https://www.home-assistant.io/docs/templating/where-to-use/#limited-templates) in the `topic` and `payload` options.

> [!note] :host{display:inline-block;vertical-align:-0.125em}span,svg{display:block} Note
...
For setup steps, YAML options, and examples for the numeric state trigger, see [Numeric state trigger](https://www.home-assistant.io/triggers/numeric_state/).

## State trigger

For setup steps, YAML options, and examples for the state trigger, see [State trigger](https://www.home-assistant.io/triggers/state/).
...
> [!tip] :host{display:inline-block;vertical-align:-0.125em}span,svg{display:block} Tip
...
Fires when a [tag](https://www.home-assistant.io/integrations/tag) is scanned. For example, an NFC tag is scanned using the Home Assistant Companion mobile application.
...
Template triggers work by evaluating a [template](https://www.home-assistant.io/docs/templating/) when any of the recognized entities change state. The trigger will fire if the state change caused the template to render ‘true’ (a non-zero number or any of the strings `true`, `yes`, `on`, `enable`) when it was previously ‘false’ (anything else).
...
> [!important] :host{display:inline-block;vertical-align:-0.125em}span,svg{display:block} Important
...
For setup steps, YAML options, and examples for the time trigger, see [Time trigger](https://www.home-assistant.io/triggers/time/).

## Time pattern trigger

For setup steps, YAML options, and examples for the time pattern trigger, see [Time pattern trigger](https://www.home-assistant.io/triggers/time_pattern/).
...
Webhook trigger fires when a web request is made to the webhook endpoint: `/api/webhook/<webhook_id>`. The webhook endpoint is created automatically when you set it as the `webhook_id` in an automation trigger. The `webhook_id` can either be a static value or computed using [limited templates](https://www.home-assistant.io/docs/templating/where-to-use/#limited-templates).
